...
public class JTetris extends JComponent {
	...
	/**
	 * Creates a new JTetris where each tetris square is drawn with the given number
	 * of pixels.
	 */
	JTetris(int pixels) {
		super();
		...
		/*
		 * Register key handlers that call tick with the appropriate constant. e.g. 'j'
		 * and '4' call tick(LEFT)
		 * 
		 * I tried doing the arrow keys, but the JSliders try to use those too, causing
		 * problems.
		 */
	 ...
	 * the current piece. Does the necessary repaints. If the placement is not
	 * possible, then the placement is undone, and the board is not changed. The
	 * board should be in the committed state when this is called. Returns the same
	 * error code as Board.place().
	 */
	...
	/**
	 * Figures a new position for the current piece based on the given verb (LEFT,
	 * RIGHT, ...). The board should be in the committed state -- i.e. the piece
	 * should not be in the board at the moment. This is necessary so dropHeight()
	 * may be called without the piece "hitting itself" on the way down.
	 * 
	 * Sets the ivars newX, newY, and newPiece to hold what it thinks the new piece
	 * position should be. (Storing an intermediate result like that in ivars is a
	 * little tacky.)
	 */
	...
	/**
	 * Called to change the position of the current piece. Each key press calls this
	 * once with the verbs LEFT RIGHT ROTATE DROP for the user moves, and the timer
	 * calls it with the verb DOWN to move the piece down one square.
	 * 
	...
	public void tick(int verb) {
		...
		/*
		 * How to detect when a piece has landed: if this move hits something on its
		 * DOWN verb, and the previous verb was also DOWN (i.e. the player was not still
		 * moving it), then the previous position must be the correct "landed" position,
	...
	/*
	 * Pixel helpers. These centralize the translation of (x,y) coords that refer to
	 * blocks in the board to (x,y) coords that count pixels. Centralizing these
	 * computations here is the only prayer that repaintPiece() and paintComponent()
	 * will be consistent.
	 * 
	...
	/**
	 * Draws the current board with a 1 pixel border around the whole thing. Uses
	 * the pixel helpers above to map board coords to pixel coords. Draws rows that
	 * are filled all the way across in green.
	 */
	...
	/**
	 * Creates the panel of UI controls -- controls wired up to call methods on the
	 * JTetris. This code is very repetitive.
	 */
	...
	/**
	 * Creates and returns a frame around the given JTetris. The new frame is not
	 * visible.
	 */
