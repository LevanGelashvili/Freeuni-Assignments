 ...
 * 
 * To make report atomic, we lock it and post before every return statement
 */
int
Report_Transfer(Bank *bank, int workerNum, AccountNumber accountNum,
                AccountAmount amount)
{ 
  pthread_mutex_lock(&bank->rep_lock);
  ...
  if (amountAbs < reportingAmount) {
    pthread_mutex_unlock(&bank->rep_lock);
...
//Actual final report method
int doActualReport(Bank* bank, int workerNum) {
  Report *rpt = bank->report;
  ...
  for (int i=0; i<bank->numWorkers-1; i++) {
    sem_post(&bank->lockIfArrived);
  }
  return err;
}
...
{
  pthread_mutex_lock(&bank->inc_lock);
    bank->workersFinished++;
  pthread_mutex_unlock(&bank->inc_lock);

  pthread_mutex_lock(&bank->check_lock);
    if(bank->workersFinished < bank->numWorkers) {
      pthread_mutex_unlock(&bank->check_lock);
      sem_wait(&bank->lockIfArrived);
    } else {
      bank->favWorker = workerNum;
      pthread_mutex_unlock(&bank->check_lock);
    }

  if(bank->favWorker == workerNum) {
    return doActualReport(bank,workerNum);
  }
}
