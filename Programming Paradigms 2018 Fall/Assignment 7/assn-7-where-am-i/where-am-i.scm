...
(define (intersection-points seq)
    (if (null? seq) '()
      (append
        (apply append (map (lambda (otherCircle) (intersect (car seq) otherCircle)) (cdr seq)))
        (intersection-points (cdr seq))
      )
    )
)

(define (distance-product pt seq)
    (apply * (map (lambda (otherPt) (dist pt otherPt)) (remove pt seq)))
)

(define (rate-points seq)
    (map (lambda (pt) (list(distance-product pt seq)pt)) seq)
)

(define (ptCmp first second)
...
(define (clumped-points seq)
    (apply append (map cdr (prefix-of-list (sort-points(rate-points seq)) (quotient (length seq) 2))))
)

(define (average-point seq)
    (let((avgX (/ (apply + (map (lambda (cir) (x cir)) seq)) (length seq)))
        (avgY (/ (apply + (map (lambda (cir) (y cir)) seq)) (length seq))))
        (list (distance-product (list avgX avgY) seq) (list avgX avgY))
    )
)
...
(define (where-am-i dists stars)
    (quicksort (map (lambda (elem) (best-estimate elem)) (all-guesses dists stars)) ptCmp)
)
